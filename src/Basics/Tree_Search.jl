...
"""
function find_name(root::T, name::S)::T  where {T<:AbstractNode, S<:AbstractString}
    store = T[]
...
* `rn` : Vector of Nodes; desired Node is pushed to this vector when found.
"""
function find_name(root::T, name::S, rn::Vector{T})::Bool where {T<:AbstractNode, S<:AbstractString}
    # if the current node is the correct one store it in rn
    if root.name === name
        push!(rn, root)
        found = true
