...
function upgma(dm::Array{Float64,2}, leaf_names::Array{String,1})
    ...
    leaves = Array{Tuple{GeneralNode,Float64,Int64},1}(undef, size(dm, 1))
...
function upgma(dm::Array{Float64,2})
    n = size(dm)[1]
    leaves = Array{Tuple{GeneralNode,Float64,Int64},1}(undef, n)
...
"""
    upgma_int(dm::Array{Float64,2},leaves::Vector{GeneralNode})
...
function upgma_int(
    dm::Array{Float64,2},
    clusters::Array{Tuple{GeneralNode,Float64,Int64},1},
)
...
function neighbor_joining(dm::Array{Float64,2}, leaf_names::Array{String,1})
    ...
    # build array of leaves from leaf names
    leaves = Array{GeneralNode,1}(undef, size(dm, 1))
    for (ind, leaf) in enumerate(leaf_names)
        new_leaf = Node(leaf)
        leaves[ind] = new_leaf
    end # end for
...
function neighbor_joining(dm::Array{Float64,2})
    n = size(dm)[1]
    leaves = Array{GeneralNode,1}(undef, n)
    # build array of dummy leaves
    for i in 1:n
        new_leaf = Node("leaf_$i")
        leaves[i] = new_leaf
...
"""
    neighbor_joining_int(dm::Array{Float64,2},leaves::Vector{GeneralNode})
...
which it can be traversed.
"""
function neighbor_joining_int(dm::Array{Float64,2}, leaves::Vector{GeneralNode})
